...
{
    0xBB, 0xCC, 0x00, 0x01, 0x00, 0x03, 0x06, 0xBC, 0xCB, 0x91,
};

static BPS_UINT8 RSP_MSG[] = 
{
    0xBB, 0xCC, 0x00, 0x10, 0x00, 0x02, 0x07, 0x00, 0x19,
};

/** pack the launch update command request
  * packet flow: MCU -> MODULE */
TEST(COMMAND_LAUNCH_UPDATE, PackRequest)
{
    ...
    BPS_UINT16 tmpLen = BPSPackLnchUpdateReq(&data, buf_tmp, MSG_BUF_SIZE-HEADER_SIZE);
    EXPECT_GT(tmpLen, 0);
...
/** pack the launch update command response 
  * packet flow: MODULE -> MCU */

TEST(COMMAND_LAUNCH_UPDATE, PackResponse)
{
    BPS_UINT8 * buf_tmp = buf;
    ...
    BPS_UINT16 tmpLen = BPSPackLnchUpdateRsp(&data, buf_tmp, MSG_BUF_SIZE-HEADER_SIZE);
    EXPECT_GT(tmpLen, 0);
    PackBPSRemainLen(buf + BPS_REMAIN_LEN_POSITION, tmpLen);
    EXPECT_NE(PackBPSChecksum(buf, MSG_BUF_SIZE), (BPS_UINT8 *)BPS_NULL);
    tmpLen += HEADER_SIZE + BPS_CHECKSUM_SIZE;
...
// 
/** parse the launch update command request
  * packet flow: MODULE <- MCU */
TEST(COMMAND_LAUNCH_UPDATE, ParseRequest)
{
    BPS_WORD size = sizeof(REQ_MSG);
    BPSCmdLnchUpdateReq data;
    EXPECT_GT(BPSParseLnchUpdateReq(&data, REQ_MSG+BPS_CMD_WORD_POSITION+1, size), 0);
}

/** parse the launch update command response 
  * packet flow: MCU <- MODULE */
TEST(COMMAND_LAUNCH_UPDATE, ParseResponse)
{
    BPS_WORD size = sizeof(RSP_MSG);
    BPSCmdLnchUpdateRsp data;
    EXPECT_GT(BPSParseLnchUpdateRsp(&data, RSP_MSG+BPS_CMD_WORD_POSITION+1, size), 0);
    EXPECT_EQ(data.retCode, RET_CODE);
}

/** parse(DYN) the launch update command request
  * packet flow: MODULE <- MCU */
TEST(COMMAND_LAUNCH_UPDATE, ParseRequestDyn)
{
    BPS_WORD size = sizeof(REQ_MSG);
    BPSCmdLnchUpdateReq data;
...
/** parse(DYN) the launch update command response 
  * packet flow: MCU <- MODULE */
TEST(COMMAND_LAUNCH_UPDATE, ParseResponseDyn)
{
    BPS_WORD size = sizeof(RSP_MSG);
    BPSCmdLnchUpdateRsp data;
